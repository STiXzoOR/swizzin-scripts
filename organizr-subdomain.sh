...
# Generate apps include file (excludes panel and organizr)
...
# Auto-generated by organizr-subdomain.sh
# Apps to include in Organizr subdomain (excludes panel)
HEADER

	# Include all app configs except panel and organizr
	for conf in /etc/nginx/apps/*.conf; do
		[ -f "$conf" ] || continue
		local name
		name=$(basename "$conf" .conf)
		case "$name" in
		panel | organizr) continue ;;
		esac
