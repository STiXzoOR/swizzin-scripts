...
configure_network() {
    ...
    local current_ports
    current_ports=$(cat /proc/sys/net/ipv4/ip_local_port_range | tr '\t' '-')
    echo_info "Current port range: $current_ports"

    if [[ "$current_ports" != "1024-65535" ]] && [[ "$current_ports" != "1024 65535" ]]; then
        echo_info "Expanding ephemeral port range to 1024-65535..."
        sysctl -w net.ipv4.ip_local_port_range="1024 65535" > /dev/null 2>&1
        echo_success "Port range expanded"

        # Add to sysctl.d if not present
        if ! grep -q "ip_local_port_range" /etc/sysctl.d/99-streaming.conf 2>/dev/null; then
            echo "net.ipv4.ip_local_port_range = 1024 65535" >> /etc/sysctl.d/99-streaming.conf
        fi
    else
        echo_success "Port range already optimal"
    fi

    # Enable RPS (Receive Packet Steering)
    local rps_entries
    rps_entries=$(cat /proc/sys/net/core/rps_sock_flow_entries 2>/dev/null || echo "0")
    echo_info "Current RPS sock flow entries: $rps_entries"

    if [[ "$rps_entries" == "0" ]]; then
        echo_info "Enabling Receive Packet Steering..."
        sysctl -w net.core.rps_sock_flow_entries=262144 > /dev/null 2>&1 || true
        echo_success "RPS enabled"
    else
        echo_success "RPS already enabled"
    fi
}
