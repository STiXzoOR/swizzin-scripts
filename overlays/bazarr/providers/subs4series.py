...
# Cache TTL for CF clearance cookies (10 minutes)
_CF_COOKIE_TTL = 600
...
class _FlareSolverrResponse(object):
    """Minimal response object for FlareSolverr-proxied requests."""
    ...
    def raise_for_status(self):
        if self.status_code >= 400:
            raise Exception("FlareSolverr proxy returned HTTP %d" % self.status_code)
...
class Subs4SeriesProvider(Provider):
    ...
    def initialize(self):
        ...
        # Detect and use FlareSolverr/Byparr for Cloudflare Turnstile bypass
        self._detect_flaresolverr()
        if self._flaresolverr_available:
            self._obtain_cf_clearance()

    def terminate(self):
        self.session.close()

    def _detect_flaresolverr(self):
        """Auto-detect FlareSolverr/Byparr at localhost:8191 or FLARESOLVERR_URL."""
        self._flaresolverr_url = os.environ.get('FLARESOLVERR_URL', 'http://localhost:8191')
        try:
            resp = requests.get(self._flaresolverr_url + '/health', timeout=5)
            self._flaresolverr_available = resp.status_code == 200
        except Exception:
            try:
        ...
        if self._flaresolverr_available:
            logger.info('Subs4series: FlareSolverr/Byparr detected at %s', self._flaresolverr_url)
        else:
            logger.debug('Subs4series: No FlareSolverr/Byparr available at %s', self._flaresolverr_url)

    def _obtain_cf_clearance(self):
        """Use FlareSolverr/Byparr to obtain Cloudflare clearance cookies."""
        # Check cache first
        cached = region.get("subs4series_cf_cookies")
        if cached is not NO_VALUE:
            cookies, user_agent = cached
            self._apply_cookies(cookies, user_agent)
            return True
        ...
        try:
            logger.info('Subs4series: Requesting CF clearance from FlareSolverr')
            resp = requests.post(self._flaresolverr_url + '/v1', json={
                "cmd": "request.get",
                "url": self.server_url,
                "maxTimeout": 60000
            }, timeout=70)
            ...
            if resp.status_code != 200:
                logger.warning('Subs4series: FlareSolverr returned status %d', resp.status_code)
                return False
            ...
            user_agent = solution.get('userAgent', self.session.headers.get('User-Agent'))

            if 'cf_clearance' in cookies:
                self._apply_cookies(cookies, user_agent)
                # Cache for 10 minutes
                region.set("subs4series_cf_cookies", (cookies, user_agent))
                logger.info('Subs4series: Obtained CF clearance via FlareSolverr')
                return True
            else:
                logger.warning('Subs4series: FlareSolverr did not return cf_clearance cookie')
                return False
        except Exception as e:
            logger.warning('Subs4series: FlareSolverr request failed: %s', e)
            return False

    def _apply_cookies(self, cookies, user_agent):
        """Apply cookies and user-agent from FlareSolverr to our session."""
        for name, value in cookies.items():
            self.session.cookies.set(name, value)
        self.session.headers['User-Agent'] = user_agent

    def _request(self, method, url, **kwargs):
        """Wrapper that retries with fresh CF cookies on 403."""
        r = getattr(self.session, method)(url, **kwargs)
        if r.status_code == 403 and self._flaresolverr_available:
            logger.info('Subs4series: Got 403 for %s %s, refreshing CF clearance', method.upper(), url)
            region.delete("subs4series_cf_cookies")
            if self._obtain_cf_clearance():
                r = getattr(self.session, method)(url, **kwargs)
            elif method == 'get':
                # Fallback: route GET request directly through FlareSolverr
                r = self._flaresolverr_get(url)
        return r

    def _flaresolverr_get(self, url):
        """Route a GET request through FlareSolverr/Byparr as a fallback."""
        try:
            resp = requests.post(self._flaresolverr_url + '/v1', json={
                "cmd": "request.get",
                "url": url,
            ...
            if resp.status_code == 200:
                ...
                return _FlareSolverrResponse(
                    ...
                    solution.get('status', 200)
                )
        except Exception as e:
            logger.error('Subs4series: FlareSolverr direct proxy failed: %s', e)

        return _FlareSolverrResponse('', 403)
    ...
    def _get_suggestions(self, title):
        ...
        r = self._request('get', self.server_url + self.search_url.format(title),
    ...
    def query(self, show_id, series, season, episode, title):
        ...
        r = self._request('get', page_link, timeout=10)
        r.raise_for_status()
    ...
    def download_subtitle(self, subtitle):
        if isinstance(subtitle, Subs4SeriesSubtitle):
            ...
            while tries <= 3:
                tries += 1
                r = self._request('get', subtitle.download_link,
                                  headers={'Referer': subtitle.page_link}, timeout=10)

                if r.status_code == 403:
                    logger.warning('Subs4series: GET download page returned 403 (try %d/3)', tries)
                    continue

                if "g-recaptcha" in r.text or "grecaptcha" in r.text:
                    logger.info('Subs4series: Solving captcha via anti-captcha service')

                    site_key = None
                    for g, s in (("g-recaptcha-response", r'g-recaptcha.+?data-sitekey=\"(.+?)\"'),
                                 ("recaptcha_response", r'grecaptcha.execute\(\'(.+?)\',')):
                        match = re.search(s, r.text)
                        if match:
                    ...
                    # No captcha on page (CF was handled by _request/Byparr), proceed
                    break

            if r.status_code != 200:
                logger.error('Subs4series: Could not fetch download page (status %d)', r.status_code)
                return
            ...
            r = self._request('post', download_url, data=data,
                              headers={'Referer': subtitle.download_link},
                              allow_redirects=True, timeout=10)
            if r.status_code == 403:
                raise Exception("Subs4series: download blocked (403). CF clearance expired.")
    ...
    def apply_anti_block(self, subtitle):
        self._request('get', self.server_url + self.anti_block_1, headers={'Referer': subtitle.download_link}, timeout=10)
        self._request('get', self.server_url + self.anti_block_2, headers={'Referer': subtitle.download_link}, timeout=10)
