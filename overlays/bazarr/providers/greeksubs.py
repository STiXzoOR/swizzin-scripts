...
class GreekSubsSubtitle(Subtitle):
    ...
    def __init__(self, language, page_link, version, uploader, referer, subtitle_id, source_page_url=None):
        ...
        self.source_page_url = source_page_url or referer
        self.matches = set()
...
class GreekSubsProvider(Provider):
    ...
    def download_subtitle(self, subtitle):
        # Re-fetch the source page to get a fresh single-use secCode token
        logger.debug('Greeksubs: Fetching fresh secCode from %s', subtitle.source_page_url)
        r = self.session.get(subtitle.source_page_url, timeout=30)

        if r.status_code != 200:
            logger.error('Greeksubs: Failed to refresh source page (status %d)', r.status_code)
            return None

        soup_page = ParserBeautifulSoup(r.content.decode('utf-8', 'ignore'), ['html.parser'])
        try:
            fresh_sec_code = soup_page.find('input', {'id': 'secCode'}).get('value')
        except Exception as e:
            logger.error('Greeksubs: Could not extract fresh secCode: %s', e)
            return None

        # Build fresh download URL with the new token
        fresh_download_url = self.server_url + 'dll/' + subtitle.subtitle_id + '/0/' + fresh_sec_code

        # Percent-encode non-ASCII chars in Referer to avoid latin-1 UnicodeEncodeError
        safe_referer = urlquote(subtitle.source_page_url, safe=':/?#[]@!$&\'()*+,;=-._~')
        r = self.session.get(fresh_download_url,
                             headers={'Referer': safe_referer},
                             timeout=30, allow_redirects=False)

        if r.status_code == 302:
            logger.critical("Greeksubs: Token exhausted even after refresh")
            return None
        ...
        try:
            ...
            download_req = self.session.post(fresh_download_url, data={'langcode': langcode,
                                                                       'uid': uid,
                                                                       'output': output,
                                                                       'dll': dll},
                                             headers={'Referer': fresh_download_url}, timeout=10)
